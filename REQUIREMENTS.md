...
- Python版は`--check-ignore [PATH ...]`で、R2へ接続せずに設定済み除外ルールの判定結果を確認できる。PATH省略時はVaultをローカル走査して除外対象を一覧表示し、PATH指定時は各Vault相対パスを個別判定する。個別判定は全件除外なら終了コード0、同期対象が1件でもあれば1を返す。この診断ではVault・R2・stateを変更しない
