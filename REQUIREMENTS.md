...
- Python版は`--check-ignore [PATH ...]`で、R2へ接続せずに設定済み除外ルールの判定結果を確認できる。PATH省略時はVaultをローカル走査し、除外対象を`IGNORE`、除外されないローカルファイルを`INCLUDE`として両方とも標準出力へ一覧表示する。`INCLUDE`はR2未照合の同期対象候補であり、PUSH確定を意味しない。PATH指定時は各Vault相対パスを個別判定し、全件除外なら終了コード0、同期対象が1件でもあれば1を返す。この診断ではVault・R2・stateを変更しない
