...
`sync.py --config <config> --check-ignore`は同じmatcherでVaultを読み取り専用走査し、除外するファイル／ディレクトリを`IGNORE`、除外されないローカルファイルを`INCLUDE`として標準出力へ表示し、R2クライアント生成前に終了する。通常表示の`IGNORE`はディレクトリ全体の除外だけを`all files`として走査を打ち切り、`.env`や個別指定ファイルはパスを1件ずつ表示する。`INCLUDE`は除外対象を含まないサブツリーを最上位のフォルダ単位で集計し、除外対象と混在するフォルダだけ再帰的に展開する。件数と集約注記は英語で表示する。`--verbose`では除外ディレクトリ配下も読み取り専用走査し、判定した全パスを列挙する。`INCLUDE`はR2未照合のためPUSH／NOOP／PULLを区別しない。`--check-ignore <path>...`では指定したVault相対パスだけを判定する。リポジトリの`justfile`は通常一覧、verbose一覧、個別判定のrecipeを提供する。
...
同期開始時は、正規化したVault絶対パスのSHA-256から一時領域内のロックファイル名を決め、そのファイルへ非待機のOS排他ロックを取得する。Windowsでは`msvcrt.locking`、iOS／Unixでは`fcntl.flock`を使用し、ロック用ファイル自体はVaultへ作らない。競合時はR2クライアント生成前に`PullError`として終了し、待機や自動再試行は行わない。ファイルの存在ではなくOSロックの保持状態で判定するため、強制終了後に残ったファイルが次回実行を妨げることはない。`finally`で明示解放し、プロセス異常終了時もOSによる解放が働く。`--check-ignore`は読み取り専用なのでロックを取得しない。この排他は同一端末・同一利用者のPython実行だけを対象とし、Node版や別端末とは共有しない。それらによるR2変更は適用前snapshot再確認で検出する。
...
Python版の論理パスはUnicode NFCへ統一する。`_safe_relative`でR2復号パスと外部入力を、ローカル走査時に各`DirEntry.name`を、`load_state`で既存stateキーをNFC化する。除外patternと保護パスも同じ表現へ揃える。`unicodeCollisionPolicy`の既定値`error`では、正規化で複数のローカル名またはstateキーが衝突した場合は`PullError`、複数remote objectが衝突した場合はremote path競合として変更前に停止する。明示値`prefer-nfc`では、衝突集合に正規化前からNFCと一致する候補がちょうど1つある場合だけそれを採用し、その他のaliasを同期mapから除外する。aliasのファイル／objectは削除・変更せず、除外総数を`unicodeAliasesIgnored`へ記録する。一意なNFC候補がない場合は安全側で停止を維持する。実ファイル参照ではNFCパスの完全一致を優先し、存在しない場合だけ各階層でNFC同値の実名を解決するため、正規化を区別するファイルシステムでも既存のNFD名を複製しない。既存remote objectを更新するPUSH／MERGEは復号前の元object keyを再利用する。
