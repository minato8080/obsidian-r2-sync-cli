...
import { classifyLocalPaths } from "./localFiles.js";
...
async function main() {
  ...
  if (args.includes("--check-ignore")) {
    ...
    const classified = await classifyLocalPaths(config.vaultPath, ignoreMatcher, args.includes("--verbose"));
    console.log(`=== 除外判定（ローカルのみ / R2通信なし）===\nvault: ${config.vaultPath}\nignoreExtra: ${config.ignoreExtra.length} patterns\n\n[IGNORE] ${classified.ignored.length} entries`);
    for (const item of classified.ignored) console.log(`  ${item}${item.endsWith("/") && !args.includes("--verbose") ? " (all files)" : ""}`);
    console.log(`\n[INCLUDE] ${classified.included.length} files`); for (const item of classified.included) console.log(`  ${item}`); return;
  }
  ...
  if (config.mode === "probe") {
    const result = await executeProbe(config, r2, cipher, ignoreMatcher, apply); report(result); console.log(JSON.stringify(result, null, 2)); process.exitCode = result.ok ? 0 : 1; return;
  }
  ...
  report(result); console.log(JSON.stringify(result, null, 2)); process.exitCode = result.ok ? 0 : 1;
}
